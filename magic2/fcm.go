...
// getNibbleMatch is the 16-sym find at decodeMatch 0x140039ae0:
// pmovmskb + add 0x10000 + bsf, adapt >>6 toward VA 0x140001f00.
func (r *rANS) getNibbleMatch(cdf []uint16) (int, error) {
	if len(cdf) < 16 {
		return 0, errBitstream
	}
	if r.off > len(r.buf) && r.x < ransL {
	...
	r.x = (end-start)*quo + (slot - start)
	sym := i - 1
	adaptMatchCDF(cdf, sym)
	r.renorm()
	return sym, nil
}

func adaptMatchCDF(cdf []uint16, sym int) {
	if sym < 0 {
		sym = 0
	}
	if sym > 15 {
...
// matchTarget is VA 0x140001f00, 16 rows of 16 u16. decodeMatch
// 0x140039ae0 adapts toward this with psraw $6.
var matchTarget = [16][16]uint16{
